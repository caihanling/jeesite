	...
	public void save(OaNotify oaNotify) {
		...
		if (oaNotify.getOaNotifyRecordList().size() > 0){
			...
			for (OaNotifyRecord oaNotifyRecord : oaNotify.getOaNotifyRecordList()) {
				System.err.println(oaNotifyRecord.toString());
			}
...
/*	public static void main(String[] args) {
		...
		oaNotify.setId("2");
		oaNotify.setType("测试类型");
		oaNotify.setTitle("测试标题");
		oaNotify.setContent("测试内容");
		oaNotify.setFiles("测试文件");
		oaNotify.setStatus("测试状态");
		oaNotify.setCreateBy(user);
		oaNotify.setCreateDate(new Date());
		oaNotify.setUpdateBy(user);
		oaNotify.setCreateDate(new Date());
		oaNotify.setRemarks("测试标记");
		oaNotify.setDelFlag("1");
		
		new OaNotifyService().save(oaNotify);
