	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="oaOvertime">
			<tr>
				...
				<c:if test="${oaOvertime.status==0}"><td style="color:#FF9900">${fns:getDictLabel(oaOvertime.status, 'activiti_status', '')}</td></c:if>
				<c:if test="${oaOvertime.status==1}"><td style="color:#3EC01D">${fns:getDictLabel(oaOvertime.status, 'activiti_status', '')}</td></c:if>
				<c:if test="${oaOvertime.status==2}"><td style="color:red">${fns:getDictLabel(oaOvertime.status, 'activiti_status', '')}</td></c:if>
				<c:if test="${oaOvertime.status==null}"><td></td></c:if>
				<shiro:hasPermission name="oa:oaOvertime:edit"><td>
    				<a href="${ctx}/oa/oaOvertime/form?id=${oaOvertime.id}&act.procInsId=${oaOvertime.procInsId}">详情</a>
